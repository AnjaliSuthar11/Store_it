...
const MobileNavigation = ({$id:ownerId,accountId,fullname,avatar,email}:Props) => {
  ...
  return (
    <header className='mobile-header'>
      <Image src="/logo2.png" alt='logo' width={120} height={52} className='h-auto'/>

      <Sheet open={open} onOpenChange={setOpen}>
  <SheetTrigger>
    <Image src="/menu.png" alt='search' width={30} height={30}/></SheetTrigger>
  <SheetContent className='shad-sheet h-screen px-3'>
      ...
      <div className='flex flex-col justify-between gap-5 pb-5'>
        ...
        <Button type='submit' className='mobile-sign-out-button' onClick={async ()=> await signOutUser()}>
          <Image src="/logout.jpg" alt='logo' width={24} height={24}/>
          <p>Logout</p>
        </Button>
      </div>
  </SheetContent>
